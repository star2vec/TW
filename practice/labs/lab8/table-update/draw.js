   ...
   if (target == "vertical") {
      ...
      for (let col=0; col<ncols; col++) {
         for (let lin=0; lin<nrows; lin++) {
            ...
            let culoare = colors[Math.floor(col / eachcolour)];
            drawPixel(lin, col, culoare);
         }
      }
      ...
      for (let lin=0; lin<nrows; lin++) {
         for (let col=0; col<ncols; col++) {
            let cell = document.getElementsByClassName("r" + lin + " c" + col)[0];
            let culoare = colors[Math.floor(lin / eachcolour)];
            drawPixel(lin, col, culoare);
         }
      }
   }
...
window.onload = function(){
   ...
   container.addEventListener("click", function(e) {
      if (e.target.tagName === "TD") {
         ...
         let clase = cell.className.split(" ");
         let lin = parseInt(clase[0].substring(1)); 
         let col = parseInt(clase[1].substring(1));

         drawPixel(lin, col, colorInput.value);
      }
   });
   ...
   const btnJumble = document.getElementById("btnJumble");
   btnJumble.addEventListener("click", function() {
      jumble();
   });
}
